...
double battery,light_sens_val, final_pwm_x, final_pwm_y;
...
// Modular function made to abstract the process
void bno_Setup(void){
...
//Populates the necessary variables
...
// Code that is executed when the Arduino turns on
...
/* 
 *  This function passes along the proper sensor readings to the control code and
 *  reads the control pwm (or duty cycle) values.
 *  
 *  The control code is defined in the header files and C-scripts that are included
 *  in the beginning of this code. 
 *  
 *  "ert_main.c" has the same function calls that are
 *  used in this script. "ert_main.h" was created so that these functions could be called
 *  in this script. "feed_control.c" defines the function calls used in "ert_main.c", and
 *  "feedback_control.h" defines the structs used.
 *  
 */ 
...
void state_check(void){
  ...
  if(state == calibrate && light_sens_val <= threshold){
    ...
    state = control; //Allows passage into the next loop
    }
...
void encoder_read_x(double *x_read){ // future function for reading X encoder value and getting an angle
...
void encoder_read_y(double *y_read){ // future function for reading Y encoder value and getting an angle
